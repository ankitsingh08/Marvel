...
interface NoteDao {
    ...
    @Query("SELECT * FROM ${Constants.NOTES_TABLE} WHERE characterId = :characterId ORDER BY id ASC")
    ...
    @Query("DELETE FROM ${Constants.NOTES_TABLE} WHERE characterId = :characterId")
